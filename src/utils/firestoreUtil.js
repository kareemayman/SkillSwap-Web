...
export const createSkillDoc = async (skill) => {
  try {
    ...
    const skillNameArabic = await generateFromGemini(
      translateSkillToArabic(skill.skillName, category)
    )
...
export const getAllTradesByUserId = async (userId) => {
  try {
    const qSnap = await getDocs(collection(db, "trades"))
    return qSnap.docs
      .map((doc) => ({ id: doc.id, ...doc.data() }))
      .filter(
        (trade) => trade.userA == userId || trade.userB == userId
      )
  } catch (error) {
    console.error("Error getting trades by userId:", error)
    return []
  }
}
